...
interface EventFormProps {
  ...
  /** Callback when an image file is selected (for deferred upload) */
  onImageFileSelect?: (file: File | null) => void;
}

export function EventForm({ newEvent, setNewEvent, editingEvent, onSubmit, onCancel, isSubmitting = false, onImageFileSelect }: EventFormProps) {
    ...
    >
      <Card className="border-gray-300 bg-gray-50">
        ...
        <CardContent className="space-y-4">
          ...
          {/* Event Image - using ImageUploadField with deferred upload */}
          <ImageUploadField
            value={newEvent.imageUrl}
            onChange={(url) => setNewEvent((prev: any) => ({ ...prev, imageUrl: url }))}
            label="이벤트 이미지"
            deferUpload={true}
